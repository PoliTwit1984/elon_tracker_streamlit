...
def display_tweet_thread(thread):
    ...
    for i, tweet in enumerate(thread):
        ...
        if i < len(thread) - 1:
            st.markdown(
                f'<p class="replying-to">Replying to @{tweet["author"]["username"]}</p>', unsafe_allow_html=True)
...
def display_tweet_content(tweet):
    ...
    metrics_html += f'<span>🔁 {tweet["public_metrics"]["retweet_count"]}</span>'
    ...
    metrics_html += f'<span>🔖 {tweet["public_metrics"].get("bookmark_count", "N/A")}</span>'
    metrics_html += f'<span>👁️ {tweet["public_metrics"].get("impression_count", "N/A")}</span>'
    metrics_html += '</div>'
